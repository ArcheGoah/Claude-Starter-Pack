...
```bash
set -a; source /c/Users/{DEIN_USER}/Desktop/{DEIN_PROJEKT}/.env; set +a
```
...
All commands use the bundled script (absolute MSYS path for Windows):
`/c/Users/{DEIN_USER}/.claude/skills/firecrawl-web/fc.py`

## Bash Discipline (Windows/MSYS)

ALWAYS use these patterns in bash examples to avoid Windows path bugs:
...
mkdir -p /c/Users/{DEIN_USER}/Desktop/{DEIN_PROJEKT}/docs/research/scrapes

# Use /c/ not C:/ for paths on Windows MSYS bash
OUTPUT_DIR="/c/Users/{DEIN_USER}/Desktop/{DEIN_PROJEKT}/docs/research/scrapes"
```
...
for attempt in 1 2 3; do
  if python3 /c/Users/{DEIN_USER}/.claude/skills/firecrawl-web/fc.py markdown "$URL"; then
    break
  fi
  sleep $((attempt * 15))
done
...
URLS=(url1 url2 url3)
for url in "${URLS[@]}"; do
  python3 /c/Users/{DEIN_USER}/.claude/skills/firecrawl-web/fc.py markdown "$url"
...
shopt -s nullglob
mkdir -p /c/Users/{DEIN_USER}/Desktop/{DEIN_PROJEKT}/docs/research/scrapes
python3 /c/Users/{DEIN_USER}/.claude/skills/firecrawl-web/fc.py markdown "https://example.com" \
  > /c/Users/{DEIN_USER}/Desktop/{DEIN_PROJEKT}/docs/research/scrapes/example.md
```

For cleaner output without navigation and footers:

```bash
python3 /c/Users/{DEIN_USER}/.claude/skills/firecrawl-web/fc.py markdown "https://example.com" --main-only
```

## Taking Screenshots

Capture a full-page screenshot (output path must use /c/ on Windows):

```bash
shopt -s nullglob
mkdir -p /c/Users/{DEIN_USER}/Desktop/{DEIN_PROJEKT}/docs/research/screenshots
python3 /c/Users/{DEIN_USER}/.claude/skills/firecrawl-web/fc.py screenshot "https://example.com" \
  -o /c/Users/{DEIN_USER}/Desktop/{DEIN_PROJEKT}/docs/research/screenshots/page.png
```
...
```bash
shopt -s nullglob
mkdir -p /c/Users/{DEIN_USER}/Desktop/{DEIN_PROJEKT}/config/firecrawl-schemas
python3 /c/Users/{DEIN_USER}/.claude/skills/firecrawl-web/fc.py extract "https://example.com/product" \
  --schema /c/Users/{DEIN_USER}/Desktop/{DEIN_PROJEKT}/config/firecrawl-schemas/product.json
```
...
```bash
python3 /c/Users/{DEIN_USER}/.claude/skills/firecrawl-web/fc.py search "Python 3.13 new features"
```

Limit results (each result = 1 credit):

```bash
python3 /c/Users/{DEIN_USER}/.claude/skills/firecrawl-web/fc.py search "latest React documentation" --limit 3
```
...
```bash
shopt -s nullglob
mkdir -p /c/Users/{DEIN_USER}/Desktop/{DEIN_PROJEKT}/docs/research/crawls
python3 /c/Users/{DEIN_USER}/.claude/skills/firecrawl-web/fc.py crawl "https://docs.newframework.dev" --limit 30
```
...
```bash
shopt -s nullglob
OUTPUT_DIR="/c/Users/{DEIN_USER}/Desktop/{DEIN_PROJEKT}/docs/research/crawls/example"
mkdir -p "$OUTPUT_DIR"
python3 /c/Users/{DEIN_USER}/.claude/skills/firecrawl-web/fc.py crawl "https://docs.example.com" \
...
- `.claude/skills/ffmpeg-batch/SKILL.md` — same Windows MSYS path discipline
